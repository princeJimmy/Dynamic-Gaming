...
# 0. Global switches
# ==============================================================================
# If True, also draw the full 9-panel diagnostic figure for each example.
PLOT_DETAILED = False

# ==============================================================================
# 1. Utility Functions & Parameters
# ==============================================================================
...
class GameParameters:
    ...
    def precompute(self):
        """Validate primitive inputs and precompute derived constants."""
    ...
    def breakeven_i(self) -> float:
        """Break-even premium for player i: C_i(p) = 1 when p = log_mgf / kappa."""
...
def make_params(T, ni0, nj0, ei, ej, lam_i, lam_j, ai, aj,
    ...
    """Build and precompute a GameParameters object from scalar inputs."""
    p = GameParameters()
    p.N = T
    p.ni0, p.nj0 = ni0, nj0
    p.ei, p.ej = ei, ej
...
# ==============================================================================
# 2. Core Model Functions (Paper Equations)
# ==============================================================================

def f_func(p_own: float, p_opp: float, a: float) -> float:
    """
    Retention/growth multiplier (Eq. 5):
    f_i(p_i, p_j) = exp(-a_i * (p_i - p_j) / p_j)
    """
...
def log_C_func(p: float, kappa: float, log_mgf: float) -> float:
    """
    Log of the single-period certainty-equivalent factor (Eq. 10):
    log C_i(p) = -kappa_i * p + log M_{X_i}(lambda_i)
    where kappa_i = lambda_i * (1 - e_i).
    """
...
def Z_step(p_own: float, p_opp: float, Z_future: float,
    ...
    """
    One step of the Z recursion (Eq. 15 / Eq. 39):
    Z_t = f_i(p_i, p_j) * (C_i(p_i) * exp(Z_{t+1}) - 1)
    """
...
# ==============================================================================
# 3. Closed-Loop Solver (Backward Induction, Section 3.2.5)
# ==============================================================================
...
def analytical_best_response(p_opp: float, Z_future: float,
    ...
    """
    Analytical best response derived from the FOC of the stage game.

    From dg_i/dp_i = 0 where g_i = f_i(p_i,p_j) * [C_i(p_i)*exp(Z)-1]:
...
def solve_stage_nash(Z_future_i: float, Z_future_j: float,
    ...
    """
    Solve the static Nash equilibrium of the period-t stage game
    using fixed-point iteration on the analytical best-response map.
    """
    # Initial guess: slightly above break-even
    pi = float(np.clip(
    ...
    for it in range(max_iter):
        prev_pi, prev_pj = pi, pj  # keep old values for an honest diff report
...
def solve_closed_loop(params: GameParameters, verbose: bool = True):
    """
    Solve the T-period closed-loop (selected Markov-perfect) equilibrium
    by backward induction (Algorithm in Section 3.2.5).

    Returns:
        prem_i, prem_j: list of equilibrium premiums
        Z_i, Z_j: list of Z values (Z_i[t] = Z_{i,t+1} for 0-indexed t,
                   with Z_i[N] = Z_{T+1} = 0)
    """
    N = params.N
    if verbose:
    ...
    prem_i = [0.0] * N
    prem_j = [0.0] * N
    # Z_i[k] stores Z_{i, k+1} (i.e., Z_i[0] = Z_{i,1}, ..., Z_i[N] = Z_{i,T+1} = 0)
    Z_i = [0.0] * (N + 1)
    Z_j = [0.0] * (N + 1)

    for t in range(N, 0, -1):
        idx = t - 1       # 0-based array index
        Zf_i = Z_i[t]     # Z_{i, t+1}
        Zf_j = Z_j[t]     # Z_{j, t+1}

        # Step 2a: solve static Nash game
        pi_star, pj_star = solve_stage_nash(Zf_i, Zf_j, params)
        prem_i[idx] = pi_star
        prem_j[idx] = pj_star

        # Step 2b: update Z (Eq. 39)
...
# ==============================================================================
# 4. Open-Loop Solver (Section 3.1)
# ==============================================================================
...
def compute_Z1_and_gradient(
    ...
    """
    Compute Z_{i,1} and its gradient w.r.t. p_own = (p_{i,1},...,p_{i,T}).

    Uses the backward recursion (Eq. 15):
      Z_t = f(p_t, q_t) * (exp(log_C(p_t) + Z_{t+1}) - 1)

    Gradient via chain rule:
      dZ_1/dp_s = [prod_{r=0}^{s-1} f_r * C_r * exp(Z_{r+1})] * dZ_s/dp_s

    Each factor dZ_r/dZ_{r+1} = f_r * C_r * exp(Z_{r+1}) > 0.
    """
    # --- Pass 1: compute all Z values (backward) ---
    Z = np.zeros(N + 1)   # Z[N] = Z_{T+1} = 0
    for t in range(N - 1, -1, -1):
        p, q = p_own[t], p_opp[t]
        Z[t] = Z_step(p, q, Z[t + 1], a, kappa, log_mgf)

    # --- Pass 2: compute gradient (forward) ---
    grad = np.zeros(N)
    ...
    for s in range(N):
        ...
        # Update chain factor: dZ_s/dZ_{s+1} = f * C * eZ
        chain_factor *= f * C_eZ
...
# ==============================================================================
# 8. Per-example full pipeline (all functionality retained)
# ==============================================================================

def run_example(title: str, params: GameParameters):
    """
    Run the complete OL/CL solve + all verifications + metrics for one example.
    Returns a dict containing everything (so the three-example plotter can use it).
    """
    print("\n" + "#" * 90)
    print(f"  {title}")
    print("#" * 90)
    ...
    # 3) Re-solve OL from five seeded random paths
    ...
    # 4) Gradient verification away from the stationary point. At the optimum,
    ...
    # 7) Optional detailed 9-panel figure (functionality retained)
    if PLOT_DETAILED:
